...
def create_dictionary(messages):
    ...
    for message in messages:
        words.extend(get_words(message))
...
def fit_naive_bayes_model(matrix, labels):
    ...
    phi_1 = (matrix_y1 + 1) / (matrix_y1.sum() + V)
    phi_0 = (matrix_y0 + 1) / (matrix_y0.sum() + V)
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    """
    # *** START CODE HERE ***
    phi_1,phi_0 = model[0],model[1]
    req_matrix = np.log(phi_1) - np.log(phi_0)
    words_list = list(dictionary.keys())
    indices = np.argsort(req_matrix)[-5:]
    rev_indices = indices[::-1]
    words = []
    for index in rev_indices:
        words.append(words_list[index])
    return words
    # *** END CODE HERE ***
